...
namespace Prova1.Controller.Tests.Common
{
    ...
    public class ApiControllerBaseTests : TestControllerBase
    {
        ...
        public void Base_Controller_HandleQueryable_ShouldBeOk()
        {
            ...
            var query = new List<ApiControllerBaseDummy>() { _dummy.Object }.AsQueryable();
            // Action
            var callback = _apiControllerBase.HandleQueryable<ApiControllerBaseDummy, ApiControllerBaseDummyQuery>(query);
            //Assert
            var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<List<ApiControllerBaseDummyQuery>>>().Subject;
            httpResponse.Content.Should().NotBeNull();
        }
