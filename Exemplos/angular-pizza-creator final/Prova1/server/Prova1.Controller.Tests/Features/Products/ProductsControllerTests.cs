...
namespace Prova1.Controller.Tests.Features.Products
{
    ...
    public class ProductsControllerTests : TestControllerBase
    {
        ...
        private Mock<ProductQuery> _productQuery;
        ...
        public void Initialize()
        {
            ...
            _productQuery = new Mock<ProductQuery>();
        ...
        public void Products_Controller_Get_ShouldOk()
        {
            ...
            var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<PageResult<ProductQuery>>>().Subject;
        ...
        public void Products_Controller_GetById_ShouldBeOk()
        {
            ...
            var id = 1;
            _productUpdateCmd.Setup(p => p.Id).Returns(id);
            _productQuery.Setup(p => p.Id).Returns(id);
            _productServiceMock.Setup(c => c.GetById(id)).Returns(_productQuery.Object);
            // Action
            IHttpActionResult callback = _productsController.GetById(id);
            // Assert
            var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<ProductQuery>>().Subject;
