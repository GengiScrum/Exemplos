...
namespace Prova1.Controller.Tests.Features.Orders
{
    ...
    public class OrdersControllerTests : TestControllerBase
    {
        ...
        private Mock<OrderQuery> _orderQuery;
        ...
        public void Initialize()
        {
            ...
            _orderQuery = new Mock<OrderQuery>();
        ...
        public void Orders_Controller_Get_ShouldOk()
        {
            ...
            var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<PageResult<OrderQuery>>>().Subject;
        ...
        public void Orders_Controller_GetById_ShouldBeOk()
        {
            // Arrange
            var id = 1;
            _orderQuery.Setup(p => p.Id).Returns(id);
            _orderServiceMock.Setup(c => c.GetById(id)).Returns(_orderQuery.Object);
            // Action
            IHttpActionResult callback = _ordersController.GetById(id);
            // Assert
            var httpResponse = callback.Should().BeOfType<OkNegotiatedContentResult<OrderQuery>>().Subject;
            httpResponse.Content.Should().NotBeNull();
            httpResponse.Content.Id.Should().Be(id);
            _orderServiceMock.Verify(s => s.GetById(id), Times.Once);
            _orderQuery.Verify(p => p.Id, Times.Once);
        }
