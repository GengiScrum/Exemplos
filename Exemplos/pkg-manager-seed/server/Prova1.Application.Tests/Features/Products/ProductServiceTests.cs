...
namespace Prova1.Application.Tests.Features.Products
{
    ...
    public class ProductServiceTests : TestServiceBase
    {
        ...
        public void Products_Service_Delete_ShouldBeOk()
        {
            ...
            _productRepositoryFake.Setup(pr => pr.Remove(productCmd.ProductIds.First())).Returns(mockIsRemoved);
            //Action
            var isProductRemoved = _service.Remove(productCmd);
            //Assert
            _productRepositoryFake.Verify(pr => pr.Remove(productCmd.ProductIds.First()), Times.Once);
        ...
        public void Products_Service_Delete_ShouldBeHandleNotFoundException()
        {
            //Arrange
            var productCmd = ObjectMother.GetProductValidToRemove();
            _productRepositoryFake.Setup(pr => pr.Remove(productCmd.ProductIds.First())).Throws<NotFoundException>();
            //Action
        ...
        [Test]
        public void Products_Service_Delete_ShouldBeHandleNotRemoved()
        {
            //Arrange
            var productCmd = ObjectMother.GetProductValidToRemove();
            var mockIsRemoved = false;
            _productRepositoryFake.Setup(pr => pr.Remove(productCmd.ProductIds.First())).Returns(mockIsRemoved);
            //Action
            var isProductRemoved = _service.Remove(productCmd);
            //Assert
            _productRepositoryFake.Verify(pr => pr.Remove(productCmd.ProductIds.First()), Times.Once);
            isProductRemoved.Should().BeFalse();
        }
